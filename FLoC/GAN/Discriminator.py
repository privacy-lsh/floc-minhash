...
class Discriminator(object):
    def __init__(self, sequence_length, num_classes, vocab_size,dis_emb_dim,filter_sizes, num_filters,batch_size,hidden_dim, start_token,goal_out_size,step_size,l2_reg_lambda=0.0):
        ...
        # D_input_y used as labels ([0,1] if positive and [1,0] if negative) for cross entropy loss  see dataloader
        ...
        with tf.name_scope('D_update'):
            ...
            D_scores, D_predictions,self.ypred_for_auc = self.classification(D_feature) # Note: classification uses features and not input needed for simhash
            ...
            self.D_params = [param for param in tf.trainable_variables() if
                             'Discriminator' or 'FeatureExtractor' in param.name] # the variable_scope in FeatureExtractor and Classification ?
