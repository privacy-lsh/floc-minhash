...
class LeakGAN(object):
    def __init__(self, sequence_length, num_classes, vocab_size,
        ...
        with tf.variable_scope('place_holder'):
            ...
            # Note: since changed SEQ_LENGTH need to put shape below to integer division aka // instead /
        ...
        #                                                dynamic_size=False, infer_shape=True, clear_after_read=False)
        real_goal_array = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length//self.step_size, # Note: Want integer division write it python 3 way
                                                        dynamic_size=False, infer_shape=True, clear_after_read=False)
        ...
        #                                                dynamic_size=False, infer_shape=True, clear_after_read=False)
        gen_o_worker_array = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length // self.step_size, # Note: integer division new syntax
                                                        dynamic_size=False, infer_shape=True, clear_after_read=False)
        ...
        #                                                dynamic_size=False, infer_shape=True, clear_after_read=False)
        sub_feature = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length // self.step_size, # NoteL new integer div syntax
                                                   dynamic_size=False, infer_shape=True, clear_after_read=False)
        ...
        def preTrain(i,x_t,g_predictions,h_tm1,input_x,h_tm1_manager,last_goal,real_goal,feature_array,real_goal_array,sub_feature,all_sub_features,all_sub_goals):
            ...
            sub_feature = tf.cond(((((i) % step_size) > 0)),
                               lambda: sub_feature,
                               lambda: (tf.cond(i > 0, lambda:sub_feature.write(i//step_size-1,tf.subtract(feature, feature_array.read(i - step_size))), # Note: new integer div / to //
                                                       lambda: sub_feature)))
            ...
            real_goal_array = tf.cond(((i) % step_size) > 0, lambda: real_goal_array,
                                                            lambda: tf.cond((i)/step_size  < self.sequence_length//step_size, # Note: new int div / => // but only for 1
                                                                        lambda :tf.cond(i>0,lambda :real_goal_array.write((i)//step_size, real_sub_goal), # Note: new int div / => //
        ...
        print(self.real_goal_array.shape) # Note: parenthesis wrap print
        ...
        ) / (self.sequence_length * self.batch_size/self.step_size) # Here want no integer div for loss ?
